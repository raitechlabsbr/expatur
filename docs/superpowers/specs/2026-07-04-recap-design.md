...
Fiel ao monólito: um modo **"Recapitulative"** dentro de `section-bookings`, não uma seção nova.
- `recap.js` injeta um `<div id="recap-view">` dentro de `#section-bookings` e um `<style>` que, com
  a classe `.recap-on` no `#section-bookings`, **esconde** as views normais (`.bk-toolbar`,
  `#bk-main-view`, `#bookings-detail`) e **mostra** `#recap-view`. (Todos esses ids/classes já
  existem no `index.html` da plataforma — confirmado.)
- Um **botão de toggle** "Recapitulative / Bookings" (na `.bk-toolbar` de Bookings) chama
  `window.recapEnable()` / `window.recapDisable()`, que alternam `window.__RECAP_ENABLED`, aplicam a
  classe, trocam o título da seção e renderizam (Recap `_render()` ou `bookingsRender()`).
...
- `index.html` — o botão de toggle "Recapitulative" na `.bk-toolbar` de `#section-bookings`.
...
2. Abrir **Bookings** → botão **Recapitulative** entra no modo relatório; **Bookings** volta.
